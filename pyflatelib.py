    ...
    while True:
        ...
        if blocktype == 0:
            ...
            if blocktype == 1: # Static Huffman
                ...
                # print("Dynamic Huffman tree: length codes: %s, distances codes: %s, code_lengths_length: %s" % (len_codes, distances, code_lengths_length))
...
import array
def decode(stream):
    if isinstance(stream, (list, tuple, array.array)):
        stream = io.BytesIO(bytes(stream))
    elif isinstance(stream, (bytes, bytearray)):
        stream = io.BytesIO(stream)
    field = RBitfield(stream)
    magic = field.readbits(16)
    if magic == 0x1f8b: # GZip
        dezipped = gzip_main(field)
    elif magic == 0x425a: # BZip2
        dezipped = bzip2_main(field)
    else:
        raise "Unknown file magic "+hex(magic)+", not a gzip/bzip2 file"
    return out

def decode_file(from_path, to_path="out"):
    with open(from_path) as stream, open(to_path, 'w') as output:
        out_data = decode(stream)
